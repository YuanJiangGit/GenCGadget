...
public:
    ...
    // jy
    virtual std::ostream& printVal(std::ostream& os, KeyT key)
    {
        os << key;
        return os;
    }
    ...
    // \return - error state: true if there's an error, false otherwise
    virtual bool checkNodeLocat(std::ostream& os, NodeT *node)
    {
        bool err = false;
    ...
    void dumpSubgraphNodes(DependenceGraph<NodeT> *sub, bool with_nodes = true)
    {
        if (with_nodes) {
            // dump all nodes, to get it without BBlocks
            // (we may not have BBlocks or we just don't want
            // to print them
            for (auto& I : *sub) {
                dump_node_in_src(I.second, 2);
            }
            
            if (dumpedGlobals.insert(sub->getGlobalNodes().get()).second) {
                for (auto& I : *sub->getGlobalNodes()) {
                    dump_node_in_src(I.second, 2, "GLOB");
                }
            }
        }
...
private:
    ...
    /**
     与源文件有对应的节点，写入到.code.gadgets文件
     @param node <#node description#>
     @param ind <#ind description#>
     @param prefix <#prefix description#>
     */
    void dump_node_in_src(NodeT *node, int ind = 1, const char *prefix = nullptr)
    {
        bool err = false;
//        unsigned int dfsorder = node->getDFSOrder();
//        unsigned int bfsorder = node->getDFSOrder();
        ...
        // check if the node is OK, and if not
        // highlight it
        err = checkNodeLocat(out, node);
        
        if(slice_id!=0&&!err){
            printVal(out, node->getKey());
        }
