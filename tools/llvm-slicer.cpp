...
llvm::cl::opt<bool> code_gadget("code-gadget",
                            llvm::cl::desc("Dump code gadgets to file (default=false)."),
                            llvm::cl::init(false), llvm::cl::cat(SlicingOpts));
...
public:
    ...
    void dumpToDot(const char *suffix = nullptr) {
        ...
        if (suffix)
            replace_suffix(fl, suffix); //替换后缀 fact.sliced.dot
        else
        ...
        if (bb_only) {
            ...
            debug::LLVMDG2Dot dumper(dg, dump_opts, fl.c_str());  //执行这个
            dumper.dump();
        }
    }
    
    void sliceToFile(const char *suffix = nullptr) {
        /**
         将LLVM IR切片以及对应的源文件中的location写入文件 (jiang)
         */
        // compose new name
        std::string fl(options.inputFile);
        if (suffix)
            ...
            replace_suffix(fl, ".gadget");
        
        errs() << "[Vul-code-gadgets] Dumping LLVM IR slicing to " << fl << "\n";
        
        debug::LLVMDG2Dot dumper(dg, dump_opts, fl.c_str());  //执行这个
        dumper.dump_code_gadget();  // 重写dump 方法
    }
...
public:
    ...
    void annotate(const std::set<LLVMNode *> *criteria = nullptr)
    {
        ...
        if (options.dgOptions.PTAOptions.analysisType
                == LLVMPointerAnalysisOptions::AnalysisType::fi)  // 执行的这个
        ...
        if (options.dgOptions.PTAOptions.fieldSensitivity == Offset::UNKNOWN)  // 执行的这个
            module_comment += "full\n\n";
        else
            module_comment
...
int main(int argc, char *argv[])
{
    ...
    /// ---------------
    // slice the code, 这里的类作用域::代表全局的意思
    /// ---------------
    ...
    if (annotator.shouldAnnotate()) //Saving IR with annotations to
    ...
    // slice the graph,会将不是slicing的节点删除掉，因此再次调用dumper.dumpToDot剩下的就只有slicing的节点
    if (!slicer.slice()) {
        errs() << "ERROR: Slicing failed\n";
        return 1;
    }

    if(code_gadget){ // code_gadget自定义指令
        dumper.sliceToFile(".sliced.gadget");
    }
